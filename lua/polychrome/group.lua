...
local function is_valid_color_value(value)
    return type(value) == "string" or type(value) == "number" or Color.is_color(value)
end
...
local function is_valid_link(value)
    return type(value) == "table" and value.is_highlight_group == true
end
...
---@param value any
---@return Group
function M.Group:set(key, value)
    ...
    if key == "gui" then
        ...
        local flags = parts:filter(function(flag)
            return flag ~= "gui"
        end)
...
---Is a link present along with other attributes?
---@return boolean
function M.Group:is_combo_link()
...
---Is a single link present without any other attributes?
---@return boolean
function M.Group:is_pure_link()
    return vim.tbl_count(self.links) == 1 and vim.tbl_count(self.attributes) == 0
end

---Create a new group definition from an existing highlight group fetched with `nvim_get_hl`
---@param name string
---@return Group?
...
---Fold all the links into a single table
---@return table
function M.Group:_fold_links()
    -- if we have a combo link, we unroll all the links and combine them
    return vim.iter(ipairs(self.links))
        :map(function(_, link)
        ...
        :fold({}, function(acc, hl)
            return vim.tbl_extend("force", acc, hl)
        end)
end

---Convert the group definition to a table suitable for `nvim_set_hl`
---@param resolve boolean? Should we resolve/unroll nested links?
---@return table
function M.Group:to_hl(resolve)
